...
{
	if (!g_pSharedData || !g_pmodeMgr || !g_renderer || !*g_renderer)
		return;
		...
		if (p_gamemode && pcmode->m_curModeType == 1 && p_gamemode->m_world && p_gamemode->m_view && p_gamemode->m_world->m_attr)
		{
			...
			for (std::list<CGameActor*>::iterator it = actorList.begin(); it != actorList.end(); it++)
			{
				...
				if (pGameActor && !pGameActor->m_isPc)
				{
					...
					char szInfo[256];
					sprintf_s(szInfo, "(%ld,%ld)\nlv = %d\njob = %d\nm_npcId = %d", 
						cx, cy, pGameActor->m_clevel, (int)*(pGameActor->pJob()), pGameActor->m_npcId);
					...
					m_pSFastFont->DrawText((LPSTR)szInfo, sx, sy, D3DCOLOR_ARGB(255, 255, 255, 255), 2, NULL);
				}
			}
		}
	...
	// Restore render state AFTER all drawing
...
//		"50"                      ///< push    eax; hModule
		...
		::VirtualQuery((LPBYTE)mbi.BaseAddress + mbi.RegionSize, &mbi_data, sizeof(mbi_data)))
	{
		...
		DEBUG_LOGGING_NORMAL(("Scanning for CConnection s_wsSend/Recv..."));
		...
		DEBUG_LOGGING_NORMAL(("Scanning for CGameActor job offset..."));
		...
		DEBUG_LOGGING_NORMAL(("Scanning for g_pmodeMgr (Types A-G)..."));
		for (UINT ii = 0; ii < mbi.RegionSize - 1000; ii++)
		{
			LPBYTE pBase = (LPBYTE)mbi.BaseAddress;
		...
		if (g_renderer == NULL)
			DEBUG_LOGGING_NORMAL(("FAILED to find g_renderer signature!"));
		else
			DEBUG_LOGGING_NORMAL(("SUCCESS: *g_renderer found at %08X", g_renderer));
