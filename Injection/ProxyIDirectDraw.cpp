...
CProxyIDirectDraw7::CProxyIDirectDraw7(IDirectDraw7 *ptr):m_Instance(ptr), CooperativeLevel(0), PrimarySurfaceFlag(0), TargetSurface(NULL)
{
	...
	DEBUG_LOGGING_NORMAL(("IDirectDraw7::Create."));
}
...
HRESULT CProxyIDirectDraw7::Proxy_QueryInterface(THIS_ REFIID riid, LPVOID FAR *ppvObj)
{
	...
	{
		DEBUG_LOGGING_NORMAL(("CProxyIDirectDraw7::QueryInterface(IID_IDirect3D7) request."));
...
HRESULT CProxyIDirectDraw7::Proxy_CreateSurface(LPDDSURFACEDESC2 SurfaceDesc, LPDIRECTDRAWSURFACE7 FAR *CreatedSurface, IUnknown FAR *pUnkOuter)
{
...
HRESULT CProxyIDirectDraw7::Proxy_SetCooperativeLevel(HWND hWnd, DWORD dwFlags)
{
	...
	if (g_pRoCodeBind)
		g_pRoCodeBind->InitWindowHandle(hWnd);
...
{
	DEBUG_LOGGING_NORMAL(("CProxyIDirect3D7::CreateDevice2()"));
...
HRESULT CProxyIDirect3DDevice7::Proxy_BeginScene(void)
{
	...
	if (m_firstonce)
	{
		...
		if (g_pRoCodeBind)
		{
			DEBUG_LOGGING_NORMAL(("CRoCodeBind::Init triggering sigscan..."));
			g_pRoCodeBind->Init(m_Instance);
		}
	}
